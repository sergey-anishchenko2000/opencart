...
class ControllerCatalogDistributors extends Controller {
	...
	protected function getList() {
		...
		$finder_total = $this->model_catalog_distributors->getFinders();
	...
	protected function getForm() {
		...
		if (isset($this->request->get['distributor_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$finder_info = $this->model_catalog_distributors->getFinder($this->request->get['distributor_id']);
		}
		...
		} elseif (!empty($finder_info)) {
			$data['name'] = $finder_info['name'];
		} else {
			$data['name'] = '';
		}
		...
		if (isset($this->request->post['location_id'])) {
			$data['distribution_location_id'] = $this->request->post['location_id'];
		} elseif (!empty($finder_info)) {
			$data['distribution_location_id'] = $finder_info['location_id'];
		} else {
			$data['distribution_location_id'] = 0;
		}

		if (isset($this->request->post['image'])) {
			$data['image'] = $this->request->post['image'];
		} elseif (!empty($finder_info)) {
			$data['image'] = $finder_info['image'];
		} else {
			$data['image'] = '';
		}
