...
class ControllerCSRDetail extends Controller {
	private $error = array();
    ...
    public function index()
    {
        ...
        if(isset($this->request->get['article_id'])){
            ...
            $article_categories = $this->model_CSR_article->getArticleCategories($article_info['article_id']);
            $article_galleries = $this->model_CSR_article->getArticleGalleries($article_info['article_id']);
            ...
            $data['article'] = array(
                ...
                'comments_count'   =>  $this->model_CSR_comment->getTotalCommentsForArticle($article_info['article_id']),
            );
            ...
            // Author
            $data['author'] = $this->model_CSR_author->getAuthor($article_info['author_id']);
            ...
            if(!empty($data['author'])){
                ...
                $data['author']['href'] =  $this->url->link('CSR/CSR', 'author=' . $article_info['author_id']);
            }               
            // Comments
            $data['comments'] = $this->model_CSR_comment->getComments($article_info['article_id']);
            ...
            $results = $this->model_CSR_article->getProductRelated($article_info['article_id']);
            $data['products'] = array();
            ...
            $results = $this->model_CSR_article->getArticleRelated($article_info['article_id']);
            ...
            if(!empty($results)){
                foreach ($results as $article_id) {
                    $result = $this->model_CSR_article->getArticle($article_id);
                    $thumb = false;
                    ...
                    $data['articles'][] = array(
                        ...
                        'tags' => $tags_array,
                        'thumb'     => $thumb,
                        'href'        => $this->url->link('CSR/detail', (isset($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') .'article_id=' . $result['article_id'])
                    );
                }
            }
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('text_CSR'),
            ...
            if (isset($this->request->get['path'])) {
                ...
                $path = $this->model_CSR_category->getCategoryPath($first_category['category_id']);
            }
            ...
            if ($path) {
                ...
                foreach ($parts as $path_id) {
                    ...
                    if ($category_info) {
                        $data['breadcrumbs'][] = array(
                            ...
                            'href' => $this->url->link('CSR/CSR', 'path=' . $path_new . $url)
                        );
                    }
                }
                ...
                if ($category_info) {
                    ...
                    $data['breadcrumbs'][] = array(
                        'text' => $category_info['name'],
                        'href' => $this->url->link('CSR/CSR', 'path=' . $path)
                    );
                }
            } 
            ...
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('CSR/detail', 'article_id=' . $article_info['article_id'])
            );
           ...
           $this->response->setOutput($this->load->view('CSR/article_detail/' . str_replace('.tpl', '', $this->settings['article_detail_template']), $data));
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
                ...
                if($this->model_CSR_settings->getSetting('comments_approval') == 1){
				...
				$this->model_CSR_comment->addComment($this->request->get['article_id'], $this->request->post);
