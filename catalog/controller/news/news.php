...
class ControllerNewsNews extends Controller {
    ...
    public function index()
    {
			...
			foreach ($parts as $path_id) {
				...
				if ($category_info) {
					$data['breadcrumbs'][] = array(
						...
						'href' => $this->url->link('news/news', 'path=' . $path . $url)
					);
				}
			}
		...
		if ($category_info) {
			...
			$this->document->addLink($this->url->link('news/news', 'path=' . $this->request->get['path']), 'canonical');
			...
			$data['breadcrumbs'][] = array(
				'text' => $category_info['name'],
				'href' => $this->url->link('news/news', 'path=' . $this->request->get['path'])
			);
			...
			foreach ($results as $result) {
				...
				$data['categories'][] = array(
					'name'  => $result['name'],
					'href'  => $this->url->link('news/news', 'path=' . $this->request->get['path'] . '_' . $result['category_id'] . $url)
				);
			}
        }else{
        ...
        if(!empty($results)){
            foreach ($results as $result) {
                ...
                if($article_author){
                    $article_author['href'] =  $this->url->link('news/news', 'author=' . $result['author_id']);
                }else{
                ...
                foreach($article_categories as &$category){
                    $category['href'] = $this->url->link('news/news', 'path=' . $this->model_blog_category->getCategoryPath($category['category_id']));
                }
                ...
                if ($result['tags']) {
                     ...
                     foreach ($tags as $tag) {
                          $tags_array[] = array(
                               'tag'  => trim($tag),
                               'href' => $this->url->link('news/news', 'tag=' . trim($tag))
                          );
                     }
                }
                ...
                $data['articles'][] = array(
                    ...
                    'href'        => $this->url->link('news/detail', (isset($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') .'article_id=' . $result['article_id'] . $url)
                );
            }
            ...
            $data['sorts'][] = array(
                ...
                'href'  => $this->url->link('news/news', (isset($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') . 'sort=ba.sort_order&order=ASC' . $url)
            );

            $data['sorts'][] = array(
                'text'  => $this->language->get('text_title_asc'),
                'value' => 'bad.title-ASC',
                'href'  => $this->url->link('news/news', (isset($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') . 'sort=bad.title&order=ASC' . $url)
            );
            ...
            foreach($limits as $value) {
                $data['limits'][] = array(
                    'text'  => $value,
                    'value' => $value,
                    'href'  => $this->url->link('news/news',  (isset($this->request->get['path']) ? 'path=' . $this->request->get['path'] : '') . $url . '&limit=' . $value)
                );
            }
            ...
            $pagination->url = $this->url->link('news/news',  (isset($this->request->get['path']) ? 'path=' . $this->request->get['path'] : '') . $url . '&page={page}');
            ...
            if(isset($this->request->get['ajax_request']) && $this->request->get['ajax_request'] == 1){
                return $this->response->setOutput($this->load->view('news/article_list/' . str_replace('.tpl', '', $this->settings['article_list_template']), $data));
            }
           ...
           $this->response->setOutput($this->load->view('news/news', $data));
			...
			$data['continue'] = $this->url->link('news/news');
