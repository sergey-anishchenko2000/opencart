...
<div class="post">
    <div class="post-entry">
     ...
     <?php if(!empty($articles) && $settings['article_related_status'] == 1) : ?>
         ...
         <?php
         include('catalog/view/theme/' . $config->get('theme_' . $config->get('config_theme') . '_directory') . '/template/CSR/article_related/'. $settings['article_related_template']); 
         ?>
...
<script>
    $(function(){
        ...
        $('#button-comment').on('click', function(e) {
            e.preventDefault();
            $.ajax({
                url: 'index.php?route=CSR/detail/write&article_id=<?php echo $article_id; ?>',
                type: 'post',
                dataType: 'json',
