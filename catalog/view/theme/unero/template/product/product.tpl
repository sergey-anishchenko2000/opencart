			     ...
			     <div id="product">
			      ...
			      <div class="options">
			          ...
			          <div class="form-group required">
						...
						<?php foreach ($recurrings as $recurring) { $recurring_id = $recurring['recurring_id']; ?>
							<input type="button" 
								value="<?php echo $recurring['name']; ?>" 
								onclick="setRecurringValue('<?php echo $recurring_id; ?>')" 
								class="button sack<?php echo $recurring_id; ?>" />
						<?php } ?>
						<input type="hidden" name="recurring_id"/>
...
function getRecurringDescription() {
	$.ajax({
		...
		type: 'post',
		data: $('input[name=\'product_id\'], input[name=\'quantity\'], input[name=\'recurring_id\']'),
		dataType: 'json',
		beforeSend: function() {
...
function setRecurringValue(val) {
	
	$('input[name="recurring_id"]').val(val);
	getRecurringDescription();
	console.log($('input[name="recurring_id"]').val());
}
<?php if ($recurrings) { ?>
document.addEventListener('DOMContentLoaded', function(){
	setRecurringValue('<?php echo $recurrings[0]["recurring_id"]; ?>');
});
<?php } ?>
